...
public class TodoControllerTest extends AbstractTodoAppTest
{
    ...
    public void whenGetAllTodos_thenReturnJsonArray() throws Exception
    {
        ...
        mockMvc.perform(doGet("/api/v1/todos").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
            .andExpect(jsonPath("$", hasSize(2))).andExpect(jsonPath("$[0].title", equalTo(todo1.getTitle())));
    ...
    public void whenPostTodo_thenCreateTodo() throws Exception
    {
        ...
        mockMvc
            .perform(doPost("/api/v1/todos/").contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(todo1)))
            .andExpect(status().isCreated()).andExpect(jsonPath("$.title", is(todo1.getTitle())));
